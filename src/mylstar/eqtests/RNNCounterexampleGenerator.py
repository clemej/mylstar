...
class WhiteboxRNNCounterexampleGenerator:
    def __init__(self,network,parent,partitioning,starting_examples):
        ...
        self.parent = parent
        self.starting_dict = {cex:network.submit_word(cex) for cex in starting_examples}
        return
    ...
    def _get_counterexample_from(self,words):
        ...
        for w in words:
            ...
            if len(w.letters) == 0:
                ...
                # XXXjc: Replace with query/submit instead of whiteboxrnn.submit_word?
                if not self.whiteboxrnn.submit_word(w) == self.proposed_dfa.play_word(w, self.proposed_dfa.initial_state)[0]:
                    return w
        return None
    ...
    def _initialise_unrolling(self):
        ...
        self.new_RStates = [UnrollingInfo(self.proposed_dfa.initial_state,Word([EmptyLetter()]),initial_RState,pos)]

    def _cex_from_starting_dict(self,dfa):
        for cex in self.starting_dict:
            print(dfa.play_word(cex))
            print(self.starting_dict[cex])
            if not dfa.play_word(cex)[0] == self.starting_dict[cex]:
    ...
    def counterexample(self,dfa): 
        print("guided starting equivalence query for DFA of size " + str(len(dfa.get_states())))
...
def counterexample_message(counterexample,rnn):
    return ("returning counterexample of length " + str(len(counterexample)) + ":\t\t" + str(counterexample) + 
        ", this counterexample is " + str(rnn.submit_word(counterexample)) + #("accepted" if rnn.classify_word(counterexample)==True else "rejected") + 
        " by the given RNN.")
...
class UnrollingInfo:
    ...
    def __repr__(self):
        return '{} {} {} {} {}'.format(self.explored, self.dfa_state, self.paths, self.RStates, self.accepting)
